...
import { ExperienceTableColumns } from "../components/ReleaseTableColumn";
...
const ExperienceLetterUploadPage: React.FC = () => {
  ...
  const poll = async (intervalId: NodeJS.Timeout) => {
    ...
    const url = `/experience-letter?createdAt[$gte]=${encodeURIComponent(
      from
    ...
    try {
      ...
      if (Number(response?.data?.data?.length) >= parsedData?.length || error) {
        ...
        location.href = "/dashboard/experience-letter";
      }
  ...
  const handleSubmit = async () => {
    ...
    formData.append("multipleExperienceLetterCsv", uploadedFile);
    let badRequesterror = false;

    axiosInstance
      .post("/experience-letter/upload-experience-letter-csv", formData, {
        headers: {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200 mb-6">
          ...
          <p className="text-gray-600">
            Upload and process experience letter information from Excel/CSV
            files
          </p>
        </div>
...
export default ExperienceLetterUploadPage;
