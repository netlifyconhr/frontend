...
import { PayslipTableColumns } from "../components/PayslipTableColumn";
...
async function fetchSentPayslip({
  page = 1,
  limit = 5,
  searchTerm = "",
  status = offerLetterStatus.ALL,
  month,
}: FetchSentPayslipParams): Promise<TResponse<IOfferLetter[]> | undefined> {
  try {
...
export default function usePayslip() {
  ...
  const columns = useMemo(
    () => PayslipTableColumns(setSelectedOffer, setDetailsOpen),
    []
  );
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const res = await fetchSentPayslip({
